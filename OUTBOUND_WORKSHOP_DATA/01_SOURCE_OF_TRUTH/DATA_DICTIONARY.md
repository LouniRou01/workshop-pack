...
- CRM ownership, lifecycle, customer status, suppression, and pipeline state belong in `02_SOURCE_SYSTEMS/crm_state_accounts.csv`, `crm_state_contacts.csv`, or related event files.
...
- Sequence status, lead status, lifecycle stage, engagement, email validity, next step, and CRM notes belong in `02_SOURCE_SYSTEMS/crm_state_accounts.csv`, `crm_state_contacts.csv`, or `activities.csv`.

## Relationships

- `contacts.account_id` joins to `accounts.account_id`.
- `02_SOURCE_SYSTEMS/crm_state_accounts.csv` and `crm_state_contacts.csv` contain operational account and contact state, respectively.
